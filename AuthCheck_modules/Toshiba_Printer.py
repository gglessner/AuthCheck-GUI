# Toshiba Printer Authentication Module
...
module_description = "Toshiba Printer (Printer)"
...
form_fields = [
    ...
    {"name": "hints", "type": "readonly", "label": "Hints", "default": "Admin / 123456 (e-STUDIO). TopAccess: 80/443"},
]


def authenticate(form_data):
    """
    Attempt to authenticate to Toshiba e-STUDIO MFP via TopAccess.
    """
    try:
        import requests
    except ImportError:
    ...
    try:
        ...
        # Get device info
        info_url = f"{base_url}/TopAccess/Device/Device.htm"
        info_resp = session.get(info_url, timeout=10)
        
        model = "Toshiba e-STUDIO"
        if info_resp.status_code == 200:
            import re
            match = re.search(r'e-STUDIO\s*(\w+)', info_resp.text, re.IGNORECASE)
            if match:
                model = f"e-STUDIO {match.group(1)}"
        
        # TopAccess login
        login_url = f"{base_url}/TopAccess/Login/Login.htm"
        login_data = {
            "UserName": username,
            "Password": password,
            "Action": "Login"
        ...
        return False, f"Toshiba error: {e}"
