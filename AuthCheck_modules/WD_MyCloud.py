# WD My Cloud Authentication Module
...
module_description = "WD My Cloud (NAS)"
...
form_fields = [
    ...
    {"name": "hints", "type": "readonly", "label": "Hints", "default": "admin / (blank or set). HTTP: 80, HTTPS: 443"},
]


def authenticate(form_data):
    """
    Attempt to authenticate to WD My Cloud NAS.
    """
    try:
        import requests
    except ImportError:
    ...
    try:
        ...
        # My Cloud OS 5 API login
        login_url = f"{base_url}/api/2.1/rest/local_login"
        ...
        if response.status_code == 200:
            ...
            if data.get("access_token") or data.get("token"):
                # Get device info
                info_url = f"{base_url}/api/2.1/rest/device_info"
                ...
                if info_resp.status_code == 200:
                    ...
                    model = info_data.get("model", "WD My Cloud")
                    firmware = info_data.get("firmware_version", "unknown")
        ...
        if response.status_code == 200 and "logout" in response.text.lower():
            return True, f"Successfully authenticated to WD My Cloud at {host}"
        
        return False, "Authentication failed"
            
    except Exception as e:
        return False, f"WD My Cloud error: {e}"
