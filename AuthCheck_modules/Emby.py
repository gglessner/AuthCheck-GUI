"""Emby Media Server authentication module."""

module_description = "Emby (Media)"
...
form_fields = [
    ...
    {"name": "hints", "type": "readonly", "label": "Hints", "default": "TLS: 8920, Non-TLS: 8096. API key from Dashboard > API Keys."}
]
...
def authenticate(form_data):
    ...
    try:
        ...
        if response.status_code == 200:
            info = response.json()
            return True, f"Emby authentication successful (v{info.get('Version')})"
        else:
            return False, f"HTTP {response.status_code}"
            
    except Exception as e:
        return False, f"Emby error: {str(e)}"
